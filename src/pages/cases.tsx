...
import { ALL_CASES_ITEMS, getCasesItems, ITEMS_PER_PAGE } from '@/graphql/queries/cases';
...
export const getServerSideProps = (async (context) => {
  try {
    ...
    const techArr = typeof tech === 'string' ? tech.split(',') : Array.isArray(tech) ? tech : [];

    const { data: dataCount, errors: errorsCount } = await fetchGraphQL(ALL_CASES_ITEMS);

    if (errorsCount) {
      throw new Error(errorsCount[0].message)
    }

    const queryCases = getCasesItems(ITEMS_PER_PAGE, offset, country as string | undefined);
    const { data: dataCases, errors: errorsCases } = await fetchGraphQL(queryCases);

    let filteredCases = dataCases?.allCasesItem || [];

    if (techArr.length > 0) {
      filteredCases = filteredCases.filter((item: any) =>
